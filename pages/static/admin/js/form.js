(function() {
  ...
  $(document).on("adminFormReady", () => {
    document.querySelectorAll("form").forEach((form) => {
      form.querySelectorAll("input").forEach((input) => {
        if (!input.hasAttribute("data-role")) return;
        if (input.getAttribute("data-role") != "datetimepicker") return;
        if (!input.value) return;
        let offset = new Date().getTimezoneOffset();
        let originalDate = new Date(input.value);
        originalDate.setMinutes(originalDate.getMinutes() - offset);
        input.value = moment(originalDate).format("YYYY-MM-DD HH:mm:ss");
      });
